...
import {
	CanDisable,
	mixinDisabled
} from './../../../../core/common-behaviours/disabled';
import {
...
export const SELECT_MAX_OPTIONS_DISPLAYED = Math.floor(
	SELECT_PANEL_MAX_HEIGHT / SELECT_ITEM_HEIGHT
);

/** The fixed height of the select's trigger element. */
export const SELECT_TRIGGER_HEIGHT = 48;

/**
 * Must adjust for the difference in height between the option and the trigger,
 * so the text will align on the y axis.
 */
export const SELECT_OPTION_HEIGHT_ADJUSTMENT = 0;
...
export const SELECT_MULTIPLE_PANEL_PADDING_X =
	SELECT_PANEL_PADDING_X * 1.75 + 20;

/**
 * The panel's padding on the y-axis. This padding indicates there are more
 * options available if you scroll.
 */
export const SELECT_PANEL_PADDING_Y = 0;
...
export class MdSelectChange {
	constructor(public source: SelectFieldComponent, public value: any) {}
}
...
@Component({
	...
	animations: [transformPlaceholder, transformPanel, fadeInContent],
	exportAs: 'mdSelect'
})
export class SelectFieldComponent extends _MdSelectMixinBase
   ...
   */
	...
	@Input('question') question: SelectField;
	...
	@Input()
	get floatPlaceholder(): FloatPlaceholderType {
		return this._floatPlaceholder;
	}
	set floatPlaceholder(value: FloatPlaceholderType) {
		this._floatPlaceholder =
			value || this._placeholderOptions.float || 'auto';
	}
	...
	constructor(
		...
		@Optional()
		@Inject(MD_PLACEHOLDER_GLOBAL_OPTIONS)
	...
	ngOnInit() {
		this._selectionModel = new SelectionModel<Option>(
			this.multiple,
			undefined,
			false
		);
			...
			.subscribe(() => {
				...
				if (this._control) {
					...
					Promise.resolve(null).then(() =>
						this._setSelectionByValue(this._control.value)
					);
				}
			});
	}
   ...
   */
	private _setScrollTop(): void {
		const scrollContainer = this.overlayDir.overlayRef.overlayElement.querySelector(
			'.mat-select-panel'
		);
   ...
   */
	private _selectValue(value: any, isUserInput = false): Option | undefined {
		const optionsArray = this.options.toArray();
		const correspondingOption = optionsArray.find(option => {
			return option.value != null && option.value === value;
		});

		if (correspondingOption) {
			isUserInput
				? correspondingOption._selectViaInteraction()
   ...
   */
	private _clearSelection(skip?: Option): void {
	...
	private _onSelect(option: Option): void {
   ...
   */
	...
	private _calculateOverlayPosition(): void {
		...
		if (this._selectionModel.hasValue()) {
			let selectedOptionOffset = this._getOptionIndex(
				this._selectionModel.selected[0]
			)!;

			selectedOptionOffset += this._getLabelCountBeforeOption(
			...
			this._offsetY =
				(SELECT_ITEM_HEIGHT - SELECT_TRIGGER_HEIGHT) / 2 * -1 -
   ...
   */
	private _calculateOverlayOffsetX(): void {
		...
		if (this.multiple) {
			...
			const selected =
				this._selectionModel.selected[0] || this.options.first;
			offsetX =
	...
	private _calculateOverlayOffsetY(
		...
		if (this._scrollTop === 0) {
			...
			const firstDisplayedIndex =
				this._getItemCount() - SELECT_MAX_OPTIONS_DISPLAYED;
			...
			optionOffsetFromPanelTop =
				selectedDisplayIndex * SELECT_ITEM_HEIGHT +
   ...
   */
	private _checkOverlayWithinViewport(maxScroll: number): void {
		...
		const topSpaceAvailable =
			triggerRect.top - SELECT_PANEL_VIEWPORT_PADDING;
		...
		const totalPanelHeight = Math.min(
			this._getItemCount() * SELECT_ITEM_HEIGHT,
			SELECT_PANEL_MAX_HEIGHT
		);
	...
	private _getOriginBasedOnOption(): string {
		const originY =
			Math.abs(this._offsetY) -
			SELECT_OPTION_HEIGHT_ADJUSTMENT +
			SELECT_ITEM_HEIGHT / 2;
	...
	private _handleArrowKey(event: KeyboardEvent): void {
		if (this._multiple) {
			...
			const currentActiveItem = this._keyManager.activeItem as Option;
	...
	private _getLabelCountBeforeOption(optionIndex: number): number {
		if (this.optionGroups.length) {
			...
			for (let i = 0; i < optionIndex + 1; i++) {
				if (
					options[i].group &&
					options[i].group === groups[groupCounter]
				) {
					groupCounter++;
				}
			}
